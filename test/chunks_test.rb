...
# SHA-256 of its bytes, not the SHA-1, and there is no server-wide chunk size
...
class BlocksTest < Minitest::Test
  ...
  def test_server_advertises_the_surface
    ...
    assert info.ok?
    assert_includes info["features"], "chunks"
  ...
  def test_missing_reports_only_what_is_absent
    ...
    assert client.put_chunk(library_id, chunk_id(present), present).ok?

    resp = client.missing_chunks(library_id, [chunk_id(present), absent])
  ...
  def test_missing_of_nothing_is_an_empty_array_not_null
    library_id = create_test_library
    resp = client.missing_chunks(library_id, [])
    assert resp.ok?
  ...
  def test_a_chunk_that_does_not_hash_to_its_id_is_refused
    ...
    resp = client.put_chunk(library_id, wrong_id, "a different thing")
    ...
    assert_equal [wrong_id], client.missing_chunks(library_id, [wrong_id])["missing"]
  end
  ...
  def test_an_id_of_the_wrong_shape_is_refused_before_the_store_is_touched
    ...
    resp = client.put_chunk(library_id, Digest::SHA1.hexdigest(content), content)
  ...
  def test_a_chunk_already_held_is_accepted_and_says_so
    ...
    assert_equal 201, client.put_chunk(library_id, chunk_id(content), content).status
    assert_equal 200, client.put_chunk(library_id, chunk_id(content), content).status,
  ...
  def test_a_file_uploaded_as_chunks_reads_back_whole
    ...
    missing = client.missing_chunks(library_id, ids)["missing"]
    missing.each do |id|
      assert client.put_chunk(library_id, id, parts[ids.index(id)]).ok?
    end

    created = client.create_from_chunks(library_id, "/big.bin", ids)
  ...
  def test_re_asking_after_uploading_returns_a_shorter_list
    ...
    assert_equal ids, client.missing_chunks(library_id, ids)["missing"]

    client.put_chunk(library_id, ids[0], parts[0])
    assert_equal ids[1..], client.missing_chunks(library_id, ids)["missing"]

    parts[1..].each_with_index { |p, i| client.put_chunk(library_id, ids[i + 1], p) }
    assert_equal [], client.missing_chunks(library_id, ids)["missing"]
  end
  ...
  def test_creating_from_chunks_that_are_not_there_is_424_and_names_them
    ...
    resp = client.create_from_chunks(library_id, "/nope.bin", [absent])
  ...
  def test_nothing_exists_until_the_last_call
    ...
    assert client.put_chunk(library_id, chunk_id(content), content).ok?
