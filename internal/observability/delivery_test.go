...
func TestCredentialsAreScrubbedFromReports(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/silo/v1/libraries/abc/chunks/def?token=querysecret",
	...
	if !strings.Contains(request["url"].(string), "/api/silo/v1/libraries/abc/chunks/def") {
...
func TestRequestBecomesNamedTransaction(t *testing.T) {
	...
	handler := observability.Middleware(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		observability.NameTransaction(r, "/libraries/{libraryid}/chunks/{id}")
		w.WriteHeader(http.StatusOK)
	}))
	req := httptest.NewRequest(http.MethodGet,
		"/libraries/7c8a1f2e-3b4d-4c5a-9e6f-0a1b2c3d4e5f/chunks/a94a8fe5ccb19ba61c4c0873d391e987982fbbd3", nil)
	handler.ServeHTTP(httptest.NewRecorder(), req)
	observability.Flush()
	...
	payload := envs[0].payload[0]
	if got, want := payload["transaction"], "GET /libraries/{libraryid}/chunks/{id}"; got != want {
...
func TestTransactionCarriesRealStatus(t *testing.T) {
	...
	handler.ServeHTTP(httptest.NewRecorder(), httptest.NewRequest(http.MethodGet, "/libraries/abc/chunks/def", nil))
	observability.Flush()
...
func TestHandlerPanicIsReportedAndRepanics(t *testing.T) {
	...
	func() {
		...
		handler.ServeHTTP(httptest.NewRecorder(),
			httptest.NewRequest(http.MethodGet, "/libraries/abc/chunks/def", nil))
	}()
	if !repanicked {
	...
	request, _ := event["request"].(map[string]any)
	if request == nil || !strings.Contains(request["url"].(string), "/libraries/abc/chunks/def") {
