...
// clientListing returns a client whose only surface is the repos listing,
// which is all resolveRepo reads.
func clientListing(t *testing.T, repos []client.Repo) *client.APIClient {
	t.Helper()
...
func TestALibraryCanBeNamedByItsName(t *testing.T) {
	c := clientListing(t, []client.Repo{
		{ID: otherID, Name: "Documents"},
		{ID: photosID, Name: "Photos"},
	})
	got, err := resolveRepo(c, "Photos")
	if err != nil {
...
func TestAnIDIsNotLookedUp(t *testing.T) {
	...
	got, err := resolveRepo(c, photosID)
	if err != nil {
		t.Fatalf("resolveRepo: %v", err)
	}
	if got != photosID {
...
func TestAnUnknownNameSaysSo(t *testing.T) {
	c := clientListing(t, []client.Repo{{ID: photosID, Name: "Photos"}})
	_, err := resolveRepo(c, "Videos")
	if err == nil {
