...
|---|---|---|
| `400 Bad Request` | malformed or contradictory request — missing `path`, moving the library root, a `src` equal to its `dst`, a chunk whose bytes do not hash to the id it was sent under, a chunk stream that ends without its terminator, a chunk stream carrying a frame marked absent (that status says "I do not hold this" and is a *reply*; going up it means nothing, so it is refused rather than skipped and the message says so), more than 256 ids in one `POST chunks/fetch`, a `limit` that is not a positive integer within range, a `cursor` this server did not issue | fix the request; never retry unchanged |
