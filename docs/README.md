...
| [`spec/store-format.md`](spec/store-format.md) | **specification, in progress.** The normative byte-level contract between the Go `store/` package and porter-mac's Swift port — ids, chunking, the manifest/directory/commit codecs and the content crypto, with AES-SIV names and key wrapping still to land. Test vectors live beside it in `store/testdata/vectors` |
