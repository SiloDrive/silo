...
| [`spec/store-format.md`](spec/store-format.md) | **specification, in progress.** The normative byte-level contract between the Go `store/` package and porter-mac's Swift port — ids, chunking, manifests and the content crypto so far, with the directory/commit codecs and key wrapping still to land. Test vectors live beside it in `store/testdata/vectors` |
