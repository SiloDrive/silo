...
| GET | `/api/silo/v1/account` | `{"account_id","email","has_password"}` — who this credential belongs to. `has_password` is false for an account that signs in only through an identity provider: hide "change password", whose form asks for a current password there is none of. Behind any credential; nothing here is a secret. It answers before enrolment, which `account/keys` does not, and that is what it is for: `store.WrapIdentity` binds the `account_id`, so a client cannot wrap an identity key until it knows one |
