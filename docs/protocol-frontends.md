...
goes through `GenNewCommit` (`fileserver/fileop.go:1860`) and `updateBranch`
(`fileop.go:2018`), with `fastForwardOrMerge` (`fileop.go:1905`) resolving
...
(`fileop.go:155`), `doUpload` (`fileop.go:1042`) and `postMultiFiles`
(`fileop.go:1601`) all take `(rsp, r)` and write status codes inline. The
reusable layer beneath them — `fsmgr.GetObjIDByPath`, `fsmgr.GetSeafdirByPath`,
`DoPostMultiFiles` (`fileop.go:2115`), `DelFileFromTree` (`fileop.go:2132`) —
...
- Finder sprays `._*` and `.DS_Store` at any mount. `shouldIgnoreFile`
  (`fileop.go:2372`) already exists for exactly this.
