...
| Push notifications over `/notification`, on one credential-authorized lane, with the gap-after-reconnect contract, and the account ring that covers renames and new libraries | [`protocol.md`](protocol.md) § Change notifications, [`plans/notifications-account.md`](plans/notifications-account.md) |
...
**Small things that are each an afternoon.** Structured logs behind a flag, expanded metrics coverage, and a `/healthz` that actually pings
