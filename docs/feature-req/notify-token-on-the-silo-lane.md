...
Porter's fallback path can go. See [What was done](#what-was-done) at the end.

**Asked:** 18 Aug 2026, by Porter, against 0.4.3.
...
So this began as a tidying ask. It stopped being one when Porter grew a
credential model.
...
it, a long-lived client re-mints *before* it is disconnected. Porter does not
...
client-visible change. Porter decoded every token body into a
`map[string]string` — which is correct for `{"token":…}` and for
`{"jwt_token":…}`, and fails the moment a body carries a number. Porter would
have broken on the endpoint it had itself requested, on the day it shipped,
having passed every test until then. That is Porter's bug and it is fixed, but
...
Porter. Nothing of the working tree or the running server was disturbed: the
...
## What Porter deletes when this lands

`notifyJWT`, `syncToken`, the sync-token cache and the `notifyLane` field —
about 70 lines, and with them the only place in Porter that builds its own
