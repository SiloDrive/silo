...
// floorParams is the cheapest cost this format will derive under. Legal by
// construction — that is what a floor means — which is what makes it the right
// thing for every test that needs a real derivation but not a slow one, and
// for the vectors that pin the floor itself.
func floorParams(salt [KDFSaltSize]byte) KDFParams {
...
// The bound is the guard the whole params-as-data decision rests on: the
...
//
// Both ends of it are one table, because they are one rule. The floor is the
// one everybody thinks of; the ceiling is the same attack turned around — a
// server answering a pre-login parameter request with a gigabyte does not
// weaken anything, it takes the device down.
func TestDerivationRefusesParametersOutsideTheBounds(t *testing.T) {
	for _, tc := range []struct {
		what string
		p    KDFParams
	}{
		{"memory below the floor", KDFParams{Memory: MinKDFMemory - 1, Time: DefaultKDFTime, Lanes: DefaultKDFLanes}},
		{"memory at argon2's own minimum", KDFParams{Memory: 8, Time: DefaultKDFTime, Lanes: DefaultKDFLanes}},
		{"time below the floor", KDFParams{Memory: DefaultKDFMemory, Time: MinKDFTime - 1, Lanes: DefaultKDFLanes}},
		{"lanes below the floor", KDFParams{Memory: DefaultKDFMemory, Time: DefaultKDFTime, Lanes: MinKDFLanes - 1}},
		{"memory above the ceiling", KDFParams{Memory: MaxKDFMemory + 1, Time: DefaultKDFTime, Lanes: DefaultKDFLanes}},
		{"time above the ceiling", KDFParams{Memory: DefaultKDFMemory, Time: MaxKDFTime + 1, Lanes: DefaultKDFLanes}},
		{"lanes above the ceiling", KDFParams{Memory: DefaultKDFMemory, Time: DefaultKDFTime, Lanes: MaxKDFLanes + 1}},
	} {
		if _, err := DeriveCredentials("hunter2", tc.p); !errors.Is(err, ErrKDFParams) {
			t.Errorf("%s derived anyway: %v", tc.what, err)
		}
	}
}
...
func TestArgon2idMatchesTheReferenceImplementation(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		got := argon2.IDKey([]byte(testPassword), tc.p.Salt[:],
